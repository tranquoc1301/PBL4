...
{
    $this->db->select('start, end');  // Lấy thông tin start_time và end_time
    $this->db->from('shift');  // Giả sử bảng chứa thông tin ca làm việc là 'work_shifts'
    $this->db->where('id', $shift_id);
    $query = $this->db->get();

    return $query->row_array();  // Trả về thông tin ca làm việc
}
